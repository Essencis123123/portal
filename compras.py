...
if 'logado' not in st.session_state or not st.session_state.logado:
    ...
    if menu == "📝 Requisição":
        ...
        if uploaded_file:
            if st.button("Processar Upload em Lote"):
                try:
                    ...
                    if missing_cols:
                        ...
                        for col in date_cols:
                            if col in df_upload.columns:
                                df_upload[col] = df_upload[col].apply(parse_date_from_editor)
                        
                        # Converte e trata as colunas numéricas
                        numeric_cols = ['QUANTIDADE', 'VALOR_ITEM', 'VALOR_RENEGOCIADO', 'DIAS_ATRASO', 'DIAS_EMISSAO']
                        for col in numeric_cols:
                            if col in df_upload.columns:
                                df_upload[col] = pd.to_numeric(df_upload[col], errors='coerce').fillna(0)

                        # Garante que as colunas STATUS_PEDIDO, DIAS_ATRASO, e DIAS_EMISSAO estejam consistentes
                        df_upload['STATUS_PEDIDO'] = df_upload['DATA_ENTREGA'].apply(
                            lambda x: 'ENTREGUE' if pd.notna(x) else 'PENDENTE'
                        )
                        df_upload['DIAS_ATRASO'] = df_upload.apply(
                            lambda row: (row['DATA_ENTREGA'] - row['PREVISAO_ENTREGA']).days if pd.notna(row['DATA_ENTREGA']) and pd.notna(row['PREVISAO_ENTREGA']) and (row['DATA_ENTREGA'] > row['PREVISAO_ENTREGA']) else 0,
                            axis=1
                        )
                        df_upload['DIAS_EMISSAO'] = df_upload.apply(
                        ...
                        st.session_state.df_pedidos = pd.concat([st.session_state.df_pedidos, df_upload], ignore_index=True)
                        salvar_dados_pedidos(st.session_state.df_pedidos)
                        st.success(f"Upload em lote concluído! Foram adicionadas {len(df_upload)} novas requisições.")
                        st.balloons()
                        st.rerun()
