...
import re
import mimetypes
from supabase import create_client, Client # <-- Supabase ainda precisa da biblioteca
# import toml # <-- Removido o import de toml
...
# --- Configuração de Dados e Lógica de Backend (Google Sheets e Supabase) ---
SHEET_ID = st.secrets["sheet_id"] # <-- Acessa a chave diretamente
...
supabase_url = st.secrets.supabase.url # <-- Acessa a chave diretamente
supabase_key = st.secrets.supabase.key # <-- Acessa a chave diretamente
supabase_client: Client = create_client(supabase_url, supabase_key)

# FUNÇÃO DE UPLOAD PARA O SUPABASE STORAGE
def upload_to_supabase(file, bucket_name):
    """Faz o upload de um arquivo para um bucket do Supabase Storage."""
    try:
        supabase_client.storage.from_(bucket_name).upload(path=file.name, file=file.getvalue(), file_options={'content-type': file.type})
        public_url = supabase_client.storage.from_(bucket_name).get_public_url(path=file.name)
        return public_url
    except Exception as e:
        st.session_state.last_error = f"Erro ao fazer upload para o Supabase: {e}"
        return None
                        ...
                        try:
                            ...
                            for i, reembolso in enumerate(st.session_state.reembolsos_a_enviar):
                                ...
                                for comprovante in reembolso['comprovantes']:
                                    if comprovante:
                                        link = upload_to_supabase(comprovante, "SEU_BUCKET_AQUI") # <-- Atualize com o nome do seu bucket
                                        if link:
                                            comprovante_links.append(link)
                                
                                # Adiciona a nova linha no final da planilha com os links do Supabase
                                row = [
    ...
    # --- PÁGINA: CONSULTAR REEMBOLSOS ---
        ...
        if not df_reembolsos.empty:
            ...
            def create_download_link(row):
                links = []
                for i, link in enumerate(row['ID_COMPROVANTE'].split(', ')):
                    if link.strip():
                        links.append(f'<a href="{link}" target="_blank">📥 Download {i+1}</a>')
                
                return " | ".join(links)

            # ... (código para filtrar e exibir a tabela) ...
